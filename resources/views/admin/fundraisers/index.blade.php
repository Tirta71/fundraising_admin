        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-row justify-between items-center">
                    ...
                    <div class="hidden md:flex flex-row items-center gap-x-3">
                        <form action="{{ route('adminfundraisers.update' , $fundraiser) }}" method="POST">
                            @csrf
                            @method('PUT')
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col ">
                <div class="item-card flex flex-col justify-between items-center gap-y-5">
                    ...
                    @elseif ($fundraiserStatus == 'Active')
                        <a href="{{ route('adminfundraisings.create') }}" type="submit" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                            Create a Fundraising
                        </a>
                    @else
                    <form action="{{ route('adminfundraiser.apply') }}" method="POST">
                        @csrf
                        <button type="submit" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                            Become Fundraiser
                        </button>
