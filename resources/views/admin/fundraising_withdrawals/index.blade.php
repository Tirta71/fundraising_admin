    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                @forelse ($fundraising_withdrawals as $withdrawals )
                <div class="item-card flex flex-row justify-between items-center">
                    ...
                    <div>
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">Rp {{number_format($withdrawals->amount_requested, 0,',','.')  }}</h3>
                    </div>
                    <div>
                        <p class="text-slate-500 text-sm">Date</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $withdrawals->created_at }}</h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Fundraiser</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $withdrawals->fundraiser->user->name }}</h3>
                    </div>


                     @if ($withdrawals->has_sent)
                        @if ($withdrawals->has_received)
                    ...
                    <div class="hidden md:flex flex-row items-center gap-x-3">
                        <a href="{{ route('adminfundraising_withdrawals.show', $withdrawals) }}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                            View Details
                        </a>
                    </div>
                </div>
