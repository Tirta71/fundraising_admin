            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-row justify-between items-center">
                    ...
                    <div>
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">Rp {{number_format($withdraw->amount_requested, 0,',','.')  }}</h3>
                    </div>
                    <div>
                        <p class="text-slate-500 text-sm">Date</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $withdraw->created_at }}</h3>
                    </div>

                @if ($withdraw->has_sent)
                    @if ($withdraw->has_received)
                ...
                @endif
                    ...
                    <div class="hidden md:flex flex-row items-center gap-x-3">
                        <a href="{{ route('adminmy-withdrawals.details', $withdraw) }}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                            View Details
                        </a>
                    </div>
                </div>
                ...
                @empty
                    <p>Belum ada request withdrawals terbaru</p>
                @endforelse
